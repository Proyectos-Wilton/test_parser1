...
// Parser rules
testBlock: DESCRIBE OPEN_PARENTHESIS STRING COMMA OPEN_PARENTHESIS CLOSE_PARENTHESIS ENTONCES OPEN_KEY (tagIt | tagBeforeEach | tagAfterEach |tagContext )+ CLOSE_KEY CLOSE_PARENTHESIS;

tagContext: CONTEXT OPEN_PARENTHESIS STRING COMMA OPEN_PARENTHESIS CLOSE_PARENTHESIS ENTONCES OPEN_KEY (tagIt)+ CLOSE_KEY CLOSE_PARENTHESIS;
...
tagCy: CY PUNTO(tagGet | tagWait | tagVisit | tagUrl | tagContains | tagInjectAxe | tagCheckA11y | tagOn | tagStub | tagWindow | tagTittle | tagLocation | tagPause | tagIntercept | tagWrap); 

tagGet: (GET OPEN_PARENTHESIS STRING CLOSE_PARENTHESIS)( PUNTO (tagClick | tagType | tagThen | tagShould | tagWithin | tagEq | tagFind | tagContains | tagPause | tagSelect))?;
...
tagIntercept: INTERCEPT OPEN_PARENTHESIS STRING CLOSE_PARENTHESIS (PUNTO (tagAs))?;

tagWindow: WINDOW OPEN_PARENTHESIS CLOSE_PARENTHESIS ( PUNTO (tagThen | tagIts))?;
...
tagStub: STUB OPEN_PARENTHESIS CLOSE_PARENTHESIS (PUNTO tagAs)?;
...
tagThen: THEN OPEN_PARENTHESIS OPEN_PARENTHESIS (tagParam)? CLOSE_PARENTHESIS ENTONCES OPEN_KEY (expressionExpect)* CLOSE_KEY CLOSE_PARENTHESIS (PUNTO (tagIts))?;
tagShould: SHOULD OPEN_PARENTHESIS (STRING (COMMA (STRING | NUMBER) (COMMA (STRING | NUMBER))?)?) CLOSE_PARENTHESIS (PUNTO (tagFirst | tagIts | tagAnd))?;
tagWithin: WITHIN OPEN_PARENTHESIS OPEN_PARENTHESIS CLOSE_PARENTHESIS ENTONCES OPEN_KEY (tagCy)+ CLOSE_KEY CLOSE_PARENTHESIS;

tagAnd: AND OPEN_PARENTHESIS (STRING (COMMA STRING (COMMA STRING)?)?) CLOSE_PARENTHESIS (tagAnd)*;
